...
class IdentifyRequirementsService {
  /**
     ...
     */
  static async startIdentify (identifyRequirementData) {
    try {
      const parsedIdentifyRequirementData = identifyRequirementsSchema.parse(identifyRequirementData)
      const legalBasis = await LegalBasisRepository.findByIds(parsedIdentifyRequirementData.legalBasisIds)
      if (legalBasis.length !== parsedIdentifyRequirementData.legalBasisIds.length) {
      ...
      if (missingArticlesLegalBasis.length > 0) {
        throw new ErrorUtils(400, 'Some legal basis have no associated articles', {
      ...
      if (validAspectIds.length !== parsedIdentifyRequirementData.aspectsIds.length) {
      ...
      if (!requirements) {
        throw new ErrorUtils(400, 'No requirements found for the given subject and aspects', {
      ...
      throw new ErrorUtils(500, 'Unexpected error during start identify requirements')
    }
  }
  ...
  static async linkIdentifyRequirementToLegalBasis (identifyRequirementId, legalBasisId) {
    try {
      ...
      throw new ErrorUtils(500, 'Error linking identified requirement to legal basis')
    }
  }
 ...
 */
  static async linkObligatoryArticleToRequirement (identifyRequirementId, legalBasisId, articleId) {
    try {
      ...
      throw new ErrorUtils(500, 'Error linking identified requirement to article')
    }
  }
